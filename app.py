...
def analytics():
    ...
    # Calculate win rate based on closed trades only (trades with pl_amount)
    closed_trades = sum(1 for trade in trades if trade.pl_amount is not None)
...
def calendar():
    try:
        ...
        closed_trades = monthly_stats['wins'] + monthly_stats['losses']  # Sum of wins and losses represents closed trades
        monthly_stats['win_rate'] = (monthly_stats['wins'] / closed_trades * 100) if closed_trades > 0 else 0
...
def ai_advisor():
    ...
    # Calculate win rate for context based on closed trades only
    win_rate = 0
    if trades:
        profitable_trades = sum(1 for trade in trades if trade.pl_amount and trade.pl_amount > 0)
        closed_trades = sum(1 for trade in trades if trade.pl_amount is not None)
...
def export_excel():
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        analytics = pd.DataFrame([
            ...
            {'Metric': 'Win Rate', 'Value': f"{(sum(1 for t in trades if t.pl_amount and t.pl_amount > 0) / sum(1 for t in trades if t.pl_amount is not None) * 100):.2f}%" if sum(1 for t in trades if t.pl_amount is not None) > 0 else "0%"},
